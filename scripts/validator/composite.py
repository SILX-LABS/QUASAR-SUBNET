...
# Guardrail for "silent" students. Recent live telemetry showed a model can
# emit only a few tokens, look fast, and still get full credit on length /
# degeneracy / on-policy RKL. These floors make "says almost nothing" score
# as a generation failure instead of a clean concise answer.
SILENCE_MIN_LENGTH_RATIO = float(os.environ.get("SILENCE_MIN_LENGTH_RATIO", "0.08"))
SILENCE_MIN_MEAN_TOKENS = float(os.environ.get("SILENCE_MIN_MEAN_TOKENS", "12"))
ON_POLICY_MIN_MEAN_GEN = float(os.environ.get("ON_POLICY_MIN_MEAN_GEN", "16"))
ON_POLICY_MIN_TOTAL_TOKENS = float(os.environ.get("ON_POLICY_MIN_TOTAL_TOKENS", "128"))
...
# Session 3.23 — bench variance reduction. Live benchmark item budget increased
# 70 → 90 per round so one lucky/missed item moves the crown-quality gate less.
# Bump the schema so old v29 rows are quarantined until regraded under the new
# sampling budget.
COMPOSITE_SHADOW_VERSION = 30


def active_composite_axis_weights() -> dict[str, float]:
    """Return the axes that currently participate in worst/weighted scoring."""
    ...
    if BENCH_AXES_IN_COMPOSITE:
        weights.update({k: w for k, w in BENCH_AXIS_WEIGHTS.items() if w > 0})
    if ARENA_V3_AXES_IN_COMPOSITE:
        weights.update({k: w for k, w in ARENA_V3_AXIS_WEIGHTS.items() if w > 0})
...
def active_composite_axes() -> set[str]:
    """Return active nonzero weighted axes for telemetry and veto labels."""
    return set(active_composite_axis_weights())


def active_reasoning_density_bench_axes() -> set[str]:
    """Return bench axes that may feed the reasoning-density aggregate."""
    axes: set[str] = set()
    if BENCH_AXES_IN_COMPOSITE:
        axes.update(k for k, w in BENCH_AXIS_WEIGHTS.items() if w > 0)
    if ARENA_V3_AXES_IN_COMPOSITE:
        axes.update(k for k, w in ARENA_V3_AXIS_WEIGHTS.items() if w > 0)
    return axes
...
def _mean_generation_tokens(student: dict) -> float | None:
    """Best available mean generation length for silence guardrails."""
    for key in ("think_probe", "chat_probe", "length_axis"):
        payload = student.get(key) or {}
        if not isinstance(payload, dict):
            continue
        val = payload.get("mean_gen_tokens")
        if val is None and key == "length_axis":
            val = payload.get("student_mean_gen")
        if val is None:
            continue
        try:
            return float(val)
...
def _axis_length(student: dict) -> float:
    """Two-sided length score: penalize both rambling and near-silence."""
    ...
    if ratio is None:
        try:
            student_len = float(la.get("student_mean_gen") or 0.0)
            teacher_len = float(la.get("teacher_mean_gen") or 0.0)
            if student_len > 0 and teacher_len > 0:
                ratio = student_len / teacher_len
        except (TypeError, ValueError):
            ratio = None
    if ratio is not None:
        try:
            short_score = _clamp01(float(ratio) / max(SILENCE_MIN_LENGTH_RATIO, 1e-6))
            score = short_score if score is None else min(score, short_score)
        except (TypeError, ValueError):
            pass
...
def _axis_degeneracy(student: dict) -> float:
    ...
    base = 0.4 * term_score + 0.4 * degen_score + 0.2 * sb_score

    mean_gen = _mean_generation_tokens(student)
    if mean_gen is None:
        return base
    silence_factor = _clamp01(mean_gen / max(SILENCE_MIN_MEAN_TOKENS, 1e-6))
    return base * silence_factor
...
def _axis_reasoning_density(student: dict) -> float | None:
    ...
    active_benches = active_reasoning_density_bench_axes()
    for axis_name, target in REASONING_DENSITY_TARGET_TOKENS.items():
        if axis_name not in active_benches:
            continue
        payload = student.get(axis_name) or {}
        if not isinstance(payload, dict):
            continue
...
def _axis_on_policy_rkl(student: dict, king_rkl: float | None) -> float:
    ...
    if king_rkl is None or king_rkl <= 0:
        ...
        if rkl <= 0:
            ...
            base = max(0.0, min(1.0, 0.1 / max(rkl, 1e-6)))
    elif rkl <= 0:
        base = 1.0
    else:
        base = max(0.0, min(1.0, king_rkl / rkl))

    silence_factor = 1.0
    mean_gen_len = opr.get("mean_gen_len")
    if mean_gen_len is not None:
        try:
            silence_factor = min(
                silence_factor,
                _clamp01(float(mean_gen_len) / max(ON_POLICY_MIN_MEAN_GEN, 1e-6)),
            )
        except (TypeError, ValueError):
            pass
    tokens = opr.get("tokens")
    if tokens is not None:
        try:
            silence_factor = min(
                silence_factor,
                _clamp01(float(tokens) / max(ON_POLICY_MIN_TOTAL_TOKENS, 1e-6)),
            )
        except (TypeError, ValueError):
            pass
    return base * silence_factor
...
def resolve_teacher_broken_axes(teacher_student_row: dict | None,
    ...
    # Only active nonzero-weight axes can be broken for ranking purposes.
    # Muted v28 telemetry axes can still appear in ``axes`` but must not
    # affect worst/weighted decisions or teacher-broken labels.
    applicable = active_composite_axes()
...
def compute_composite(student: dict, king_kl: float | None = None,
    ...
    # Build effective weights from one shared helper so ranking, teacher
    # sanity, and telemetry labels agree on which axes are truly active.
    effective_weights = active_composite_axis_weights()
...
def compute_king_health(
    ...
    king_broken = set(king_composite.get("broken_axes") or [])
    king_axes = king_composite.get("axes") or {}
    king_worst_axis = min(
        (
            (k, v) for k, v in king_axes.items()
            if v is not None and k in active and k not in king_broken
        ),
        key=lambda kv: kv[1],
        default=(None, None),
    )[0]
    ...
    if base_composite and base_composite.get("worst") is not None:
        ...
        base_broken = set(base_composite.get("broken_axes") or [])
        base_axes = base_composite.get("axes") or {}
        base_worst_axis = min(
            (
                (k, v) for k, v in base_axes.items()
                if v is not None and k in active and k not in base_broken
            ),
            key=lambda kv: kv[1],
            default=(None, None),
        )[0]
