...
STEPS="${ORCHESTRATOR_STEPS:-${QUASAR_ORCHESTRATOR_STEPS:-0}}"
...
export QUASAR_AUTO_FINALIZE_ASSIGNMENTS="${QUASAR_AUTO_FINALIZE_ASSIGNMENTS:-1}"
...
echo "[orchestrator] assignment_limit=${STEPS} timeout_sec=${TIMEOUT_SEC} poll=${POLL}"
echo "[orchestrator] checkpoint=${QUASAR_CHECKPOINT_MANIFEST_URI}"
echo "[orchestrator] auto_finalize_assignments=${QUASAR_AUTO_FINALIZE_ASSIGNMENTS} auto_release_checkpoints=${QUASAR_AUTO_RELEASE_CHECKPOINTS} merge_validators=${QUASAR_MERGE_VALIDATOR_HOTKEYS:-orchestrator-signer} validation_targets=${QUASAR_VALIDATION_TARGET_HOTKEYS:-${QUASAR_VALIDATOR_HOTKEYS:-merge-validators-only}} quorum=${QUASAR_MERGE_VERDICT_QUORUM:-majority}"
